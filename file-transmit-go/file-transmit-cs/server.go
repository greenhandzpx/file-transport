...
func receiveFileName(conn *net.TCPConn) {
	...
	// 阻塞式
	//var buf_blocking bytes.Buffer
	//n, err := buf_blocking.ReadFrom(conn)
	//fmt.Println("ok")
	...
	//fileName := string(buf_blocking.Bytes())
	fmt.Println("正在接收文件...")
	conn.Write([]byte("ok"))
...
func main() {
	var serverIp string
	fmt.Println("请输入服务器的ip:")
	fmt.Scan(&serverIp)
	address := net.TCPAddr{
		IP: net.ParseIP(serverIp),
		Port: 12345,
	}
	listner, err := net.ListenTCP("tcp4", &address)
	if err != nil {
	...
	for {
		...
		if err != nil {
			log.Fatal("Error when accepting", err)
		}
		fmt.Println("Remote address:", conn.RemoteAddr(), "connected")
		dealFile(conn)
	}
}